...
class AIOTECH44_EnergyCore(nn.Module):
    """
    Orchestrateur central unifiant :
    - calcul adaptatif
    - agents spécialisés
    - logique neuro-symbolique différentiable
    - élagage SCG
    - allocation mémoire différentielle
    """
    ...
    def __init__(
        ...
        # 1. Calcul adaptatif
        self.compute_gate = AdaptiveComputeGate(
            emb_dim=emb_dim
        )

        # 2. Agents spécialisés
        self.agent_allocator = DynamicAgentAllocator(
            emb_dim=emb_dim,
            num_agents=num_agents
        )

        # 3. Moteur neuro-symbolique
        self.neuro_symbolic_engine = NeuroSymbolicEngine(
            emb_dim=emb_dim,
            num_rules=num_rules
        )

        # 4. Planification différentiable
        self.beam_planner = DifferentiableBeamSearch(
            emb_dim=emb_dim
        )
        ...
        # 6. Mémoire différentielle
        self.memory_allocator = DifferentialMemoryAllocator(
            base_k=2,
            max_k=min(20, num_nodes)
        )
    ...
    def forward(
        ...
        # --------------------------------------------------
        # 1. Adaptive Compute Gate
        # --------------------------------------------------
        gated_nodes, complexity_score = self.compute_gate(
            query_emb,
            graph_nodes
        )
        ...
        # --------------------------------------------------
        weighted_context, agent_weights = self.agent_allocator(
            query_emb
        )
        ...
        # --------------------------------------------------
        logic_activation = self.neuro_symbolic_engine(
            weighted_context
        )

        logic_context = torch.matmul(
            logic_activation,
            self.neuro_symbolic_engine.rule_embeddings
        )

        fused_query = query_emb + logic_context

        # --------------------------------------------------
        # 4. Differentiable Beam Search
        # --------------------------------------------------
        raw_trajectories = self.beam_planner(
            fused_query,
            gated_nodes
        )
        ...
        # --------------------------------------------------
        # 6. Differential Memory Allocation
        # --------------------------------------------------
        (
            allocated_memory,
            budget_k,
        ...
        allocated_memory_ratio = (
            allocated_tokens / total_docs
        ) * 100.0

        allocated_tokens_total = (
            padding_mask.sum().detach()
        )

        # --------------------------------------------------
        # 7. Final causal synthesis
        # --------------------------------------------------
        graph_context = gated_nodes.mean(dim=1)

        trajectory_context = (
            surviving_trajectories.mean(dim=1)
        )

        memory_summary = (
            allocated_memory.mean(dim=1)
        )
